...
def _is_aws_endpoint(endpoint: str) -> bool:
    return "amazonaws.com" in (endpoint or "").lower()
...
def _default_aws_payload(ctx: InstallerContext) -> dict[str, Any]:
    ...
    saved_vendor = ctx.get("s3_vendor")
    ...
    suggested_bucket = ctx.get("aws_storage_bucket_name") or (f"rmbl-{slug}" if slug else "")
    endpoint = ctx.get("aws_s3_endpoint_url") or _endpoint_for_region(region)
    return {
        **base,
        "aws_access_key_id": ctx.get("aws_access_key_id") or "",
        "aws_secret_access_key": ctx.get("aws_secret_access_key") or "",
...
def _suggested_bucket(domain: str) -> str:
    slug = re.sub(r"[^a-z0-9]+", "-", domain.lower()).strip("-")[:40]
    return f"rmbl-{slug}" if slug else ""
...
def _write_env_file(ctx: InstallerContext, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Generated by RMBL Server installer",
        "DEBUG=False",
