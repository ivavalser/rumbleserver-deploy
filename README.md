...
# тестовая ветка:
./deploy/operator/install-remote.sh root@YOUR_VPS_IP feat/installer
```
...
4. Генерацию `.env` (домен, пароли БД/Redis)
5. **AWS S3** — bucket, IAM-пользователь, проверка доступа
6. Деплой (`docker compose pull && up -d`)
7. Создание Django Admin суперпользователя
8. Nginx + Let's Encrypt HTTPS
9. Финальную сводку
